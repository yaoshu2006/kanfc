...
Component({
    ...
    methods: {
        ...
        draw() {
            ...
            const {signDayNum} = this.data
            const avatarPromise = getImageInfo(avatarUrl)
                 
            // const backgroundPromise = getImageInfo("resources/shareImage/yd0" + (Math.floor(Math.random() * 5) + 1) + ".jpg") 
            const backgroundPromise = getImageInfo("https://weixin.nonot.cn/shareImage/yd05.jpg")
            ...
            Promise.all([avatarPromise, backgroundPromise])
                .then(([avatar, background]) => {
                    ...
                    ctx.fillText(
                        nickName + ",您总共坚持签到了" + signDayNum +"天",
                        canvasW / 2,
                        y + rpx2px(150 * 2),
                    )
